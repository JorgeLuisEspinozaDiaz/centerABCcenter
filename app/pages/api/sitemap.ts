...
const generateSitemap = async () => {
   try {
      ...
      const response = await axios.get('http://localhost:1337/api/multilanguage?populate=deep')
      ...
      const staticUrls = menu
         ...
         .map((menuItem: { url: string }) => ({
            url: menuItem.url.startsWith('/') ? menuItem.url.slice(1) : menuItem.url,
            changefreq: 'daily',
            priority: 0.5,
         }))

      // Obtener los datos de los blogs desde la API
      const responsetwo = await axios.get('http://localhost:1337/api/blogposts')
